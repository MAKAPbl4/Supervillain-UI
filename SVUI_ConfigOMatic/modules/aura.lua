...
local MOD = SuperVillain:GetModule('SVAura');
local MAP = SuperVillain:GetModule("SVMap");
--[[ 
########################################################## 
SET PACKAGE OPTIONS
##########################################################
...
SuperVillain.Options.args.SVAura = {
	...
	set = function(a,b)MOD:ChangeDBVar(b,a[#a]);MOD:UpdateHeader(SVUI_PlayerBuffs)MOD:UpdateHeader(SVUI_PlayerDebuffs)end,
	args = {
		intro = {
			order = 1, 
			type = "description", 
		...
		enable = {
			...
			type = "toggle", 
			name = L["Enable"],
			get = function(a)return SuperVillain.protected.SVAura[a[#a]]end,
			set = function(a,b)SuperVillain.protected.SVAura[a[#a]] = b;SuperVillain:StaticPopup_Show("RL_CLIENT")end
		},
		disableBlizzard = {
			order = 3, 
			type = "toggle", 
