...
local guildFormattedXP = gsub(join("",SuperVillain.Colors:Hex(0.75,0.9,1),GUILD_EXPERIENCE_CURRENT),": ",":|r |cffffffff",1);
local guildFormattedDailyXP = gsub(join("",SuperVillain.Colors:Hex(0.75,0.9,1),GUILD_EXPERIENCE_DAILY),": ",":|r |cffffffff",1);
local guildFormattedFaction = join("",SuperVillain.Colors:Hex(0.75,0.9,1),"%s:|r |cFFFFFFFF%s/%s (%s%%)");
...
local GuildStatEventHandler = {
	...
	["SuperVillain.Colors_UPDATE"] = function()end
};
...
local function Guild_OnClick(self, button)
	if button == "RightButton" and IsInGuild() then
		MOD.tooltip:Hide()
		local ai, aj, ak, class;
		local am = 0;
		local an = 0;
		MenuMap[2].menuList = {}
		MenuMap[3].menuList = {}
		for i = 1, #GuildStatMembers do
			menuItem = GuildStatMembers[i]
			if al[7] and al[1] ~= SuperVillain.name then 
				local ai, aj = CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS[al[9]], GetQuestDifficultyColor(al[3])
				if UnitInParty(al[1])or UnitInRaid(al[1]) then
					ak = "|cffaaaaaa*|r"
				elseif not al[11]then 
					an = an + 1;
					ak = "";
					MenuMap[2].menuList[an] = {
						text = format("|cff%02x%02x%02x%d|r |cff%02x%02x%02x%s|r %s", aj.r*255, aj.g*255, aj.b*255, al[3], ai.r*255, ai.g*255, ai.b*255, al[1], ""),
						a7 = al[1], 
						notCheckable = true, 
						func = MenuInvite
					}
				...
				MenuMap[3].menuList[am] = {
					text = format("|cff%02x%02x%02x%d|r |cff%02x%02x%02x%s|r %s", aj.r*255, aj.g*255, aj.b*255, al[3], ai.r*255, ai.g*255, ai.b*255, al[1], ak),
					a7 = al[1], 
					notCheckable = true, 
					func = MenuRightClick
				}
...
local function Guild_OnEnter(self, _, ap)
	...
	local av = SuperVillain.Colors:Hex(0.75,0.9,1)
	if GetGuildLevel() ~= 25 then
		if GuildStatXP[0] then 
			local Z, a0, a1 = unpack(GuildStatXP[0])
			MOD.tooltip:AddLine(' ')
			MOD.tooltip:AddLine(format(guildFormattedXP, TruncateNumericString(Z), TruncateNumericString(a0), a1))
		end 
	end;
	...
	if standingID ~= 8 then
		...
		barMin = 0;
		MOD.tooltip:AddLine(format(guildFormattedFaction, COMBAT_FACTION_CHANGE, TruncateNumericString(barValue), TruncateNumericString(barMax), ceil(barValue / barMax * 100)))
	end;
...
local GuildColorUpdate = function()
	...
	if currentObject ~= nil then
		Guild_OnEvent(currentObject, 'SuperVillain.Colors_UPDATE')
	end 
end;
SuperVillain.Colors:Register(GuildColorUpdate)
