...
local r, g, b = unpack(SuperVillain.Colors.class);
local ir,ig,ib = (1-(r*0.8)),(1-(g*0.8)),(1-(b*0.8));
--[[ 
########################################################## 
CONSTANTS
##########################################################
...
local BreakStuff_OnEnter = function(self)
  ...
  if BreakStuffHandler.ReadyToSmash ~= true then
    ...
    self.icon:SetGradient(unpack(SuperVillain.Colors.gradient.highlight))
  end
  GameTooltip:Show()
end;
...
local BreakStuff_OnClick = function(self)
  ...
  if BreakStuffHandler.ReadyToSmash == true then
    ...
    self.icon:SetGradient(unpack(SuperVillain.Colors.gradient.green))
  end
  GameTooltip:ClearLines()
  GameTooltip:AddLine(self.ttText)
  GameTooltip:AddLine(self.subText)
...
function BreakStuffHandler:MODIFIER_STATE_CHANGED(key)
  if(not self:IsShown() and not key and (self.ReadyToSmash == false)) then return end
...
function MOD:PLAYER_REGEN_ENABLED()
  self:UnregisterEvent('PLAYER_REGEN_ENABLED')
  MOD:BreakStuffLoader()
end;

function MOD:ConfigBreakStuffButton(texture)
  ...
  BreakStuffButton.icon:SetTexture(texture)
...
function MOD:BreakStuffLoader()
  ...
  local allowed, spells, PICK, SMITH, DE = false, {};
  local spellListing, spellName = {};
  if(IsSpellKnown(51005)) then
    spellName = GetSpellInfo(51005)
    spells[ITEM_MILLABLE] = {spellName, 1/2, 1, 1/2}
  ...
  if(IsSpellKnown(31252)) then
    ...
    spells[ITEM_PROSPECTABLE] = {spellName, 1, 1/3, 1/3}
    tinsert(spellListing,spellName)
    allowed = true
  end
  ...
  MOD:ConfigBreakStuffButton("Interface\\AddOns\\SVUI\\assets\\artwork\\Icons\\BREAKSTUFF")
    ...
    if(item and not InCombatLockdown() and (BreakStuffHandler.ReadyToSmash == true)) then
      ...
      if(spell) then
        ...
        elseif(SMITH and IsThisOpenable(link)) then
          ApplyButton(link, DoIHaveAKey(), 0, 1, 1)
        end
      end
    end
  end)
  ...
  BreakStuffHandler:SetScript("OnLeave",self.MODIFIER_STATE_CHANGED)
